...
// RGB LED pins (if you want to add later)
#define RED_PIN   2
#define PURPLE_PIN 3
#define BLUE_PIN  4
...
// RGB LED functions (for future use)
void setColor(int r, int g, int b) {
  analogWrite(RED_PIN, r);
  analogWrite(PURPLE_PIN, g);
  analogWrite(BLUE_PIN, b);
}

void flashColor(int r, int g, int b, int duration = 300) {
  setColor(r, g, b);
  delay(duration);
  setColor(0, 0, 0);
}
...
void setup() {
  ...
  // Initialize RGB pins (comment out if not using LED yet)
...
void loop() {
  ...
  // Slide timer with LED flash on transitions
  unsigned long elapsed = millis() - slideStart;
  if (currentSlide == 1 && elapsed > slide1Duration) {
    currentSlide = 2;
    slideStart = millis();
    // flashColor(0, 0, 255); // blue flash for date slide (uncomment when LED ready)
  } else if (currentSlide == 2 && elapsed > slide2Duration) {
    currentSlide = 1;
    slideStart = millis();
    // flashColor(255, 255, 255); // white flash back to time (uncomment when LED ready)
  }
  ...
  if (currentSlide == 1) {
    ...
    // Progress bar (top-left, more visual)
    display.setTextSize(1);
    int bars = map(now.second(), 0, 59, 0, 12); // 0–12 bars for better resolution
    display.setCursor(0, 0);
    for (int i = 0; i < bars; i++) display.print("|");
    
    // Add bracket around progress for style
    display.setCursor(0, 8);
    display.print("[");
    display.setCursor(bars * 6, 8); // approximate character width
    display.print("]");
